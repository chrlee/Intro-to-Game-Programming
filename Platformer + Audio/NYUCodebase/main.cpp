...
std::vector<GameState*> Instantiate() {
	...
	// Load Sounds
	states[0]->sounds.emplace_back(Mix_LoadWAV("jump.wav"));
	states[0]->sounds.emplace_back(Mix_LoadWAV("kill.wav"));
				...
				for (int x = 0; x < state->levelWidth; ++x) {
					if (state->solids[state->levelData[y][x]]) {
						...
						if (isCollidingRect(ent, pos, size)) {
							if(ent->position.y > pos.y) ent->position.y += ((pos.y + size.y / 2) - (ent->position.y - ent->size.y / 2));
